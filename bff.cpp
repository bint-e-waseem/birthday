...
// Class representing the unbreakable friendship
class Friendship {
private:
    string bestieName;
    int yearsTogether;
...
public:
    ...
    void infiniteFriendshipLoop() {
        cout << "?? Starting our Infinite Friendship Loop ??\n";
        cout << "This loop represents our bond — endless, unbreakable, and full of love.\n\n";
        // Symbolic infinite loop (with a counter for demo, press Ctrl+C to stop in real run)
        for (int i = 1; i <= 5; ++i) {  // Limited for practical display
            cout << "Loop " << i << ": Our friendship grows stronger... forever ??\n";
            this_thread::sleep_for(chrono::milliseconds(600));
        }
        cout << "\n... and it continues infinitely, just like us. ??\n\n";
    }
};

// Class for Password Protected Memory Box
class MemoryBox {
private:
...
public:
    ...
    bool unlock() {
        string input;
        cout << "\n?? PASSWORD PROTECTED MEMORY BOX ??\n";
        cout << "Enter the secret password to open our treasure chest of memories: ";
        getline(cin, input);

        if (input == accessPassword) {
            cout << "? Access Granted! Welcome to our sacred memories...\n\n";
            displayMemories();
            return true;
        } else {
            cout << "? Wrong password. Only my forever bestie knows this. ??\n";
            return false;
        }
    }
...
private:
    void displayMemories() {
        ...
        cout << "\nThese memories are locked in my heart forever. ??\n\n";
    }
};

// Function to print a big heart using loops and asterisks
void printBigHeart() {
...
int main() {
    ...
    if (username == "BFF" && password == "MY GIRL") {
        cout << "\n? Login Successful! Welcome, my forever soul sister! ??\n\n";
        ...
        cout << "the softest light that guides me through the darkness. ???\n\n";
        ...
        cout << "I’ll be looking for you in Jannah too. ??????\n\n";
        ...
        cout << "\n?? Thank you for being YOU. The world is brighter because you're in it. ??\n";
    } else {
        cout << "\n? Access Denied. Only my true BFF knows the way in. Try again with love! ??\n";
    }

    cout << "\nProgram ended with endless love. ??\n";
    return 0;
}
